""" pypy version of retina.py. Tabled until pypy gets better numpy support """
		...
		if self.type == 'uniform':
			...
			for q,r in zip(radii[1:], radii): # Difference of prev and cur radii
				for theta in encircle:
					...
					'circle', int(np.round((q-r)/float(2.25)+1)), outer=(q == radii[-1])))
...
def main():
	...
	start = np.floor(image_size * 0.5 * \
	((image_size - 2*radius)/float(image_size)) + radius)
	S = logp.sample(512, 512, img)
